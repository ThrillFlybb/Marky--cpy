...
const TextEditor = () => {
  ...
  async function preview() {
    try {
      const data = await postJson("/api/compiler/preview", { text: Markdown })
      if (!data.success || !data.ast) {
        setHtml("<p>" + (data.message || "Preview failed") + "</p>")
        return
      }
      const htmlRenderer = new HtmlRenderer()
      const titleSafe = htmlRenderer.escape(Title)
      setHtml(
        "<h1 class='text-4xl font-semibold text-center mt-6 mb-6 text-gray-900'>" +
          titleSafe +
          "</h1>" +
          htmlRenderer.render(data.ast)
      )
    } catch (e) {
      setHtml("<p>" + (e.message || "Preview error") + "</p>")
    }
  }
